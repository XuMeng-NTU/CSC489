...
public class DMKD_2013_Results {
    ...
    //Set this to the file path where the datasets are stored.
    ...
    private static AssesmentType[] assesmentTypes = {
        ...
        AssesmentType.TRAIN_TEST,   // Otoliths
    ...
    private static LegacyShapeletTransform shapeletFilter;
    private static LegacyShapeletTransform shapeletFilterCV;
    ...
    private static void loadData(){
        ...
        for(int i=0; i<fileNames.length; i++){
            ...
            if(assesmentTypes[i] == AssesmentType.TRAIN_TEST){
                instancesTrain[i] = LegacyShapeletTransform.loadData(filePaths[i]+fileNames[i]+"_TRAIN.arff");
                instancesTest[i] = LegacyShapeletTransform.loadData(filePaths[i]+fileNames[i]+"_TEST.arff");
            }else if(assesmentTypes[i] == AssesmentType.LOOCV){
                instancesTrain[i] = LegacyShapeletTransform.loadData(filePaths[i]+fileNames[i]+".arff");
    ...
    private static double classiferAccuracyTrainTest(int classifierIndex, int dataIndex, boolean useTransformedData){
        ...
        if(useTransformedData){
            //Initialize filter
            try{
                shapeletFilter = LegacyShapeletTransform.createFilterFromFile(filePaths[dataIndex]+fileNames[dataIndex]+"_TRAIN_TRANS.txt", instancesTrain[dataIndex].numAttributes()/2);
                shapeletFilter.supressOutput();
            }catch (Exception e){
                shapeletFilter = new LegacyShapeletTransform();
    ...
    private static double classiferAccuracyLOOCV(int classifierIndex, int dataIndex, boolean useTransformedData){
        ...
        for (int n = 0; n < data.numInstances(); n++) {
            ...
            if(useTransformedData){
                //Initialize filter
                try{
                    shapeletFilterCV = LegacyShapeletTransform.createFilterFromFile(filePaths[dataIndex]+fileNames[dataIndex]+"_TRANS_"+n+".txt", instancesTrain[dataIndex].numAttributes()/2);
                    shapeletFilterCV.supressOutput();
                }catch (Exception e){
                    shapeletFilterCV = new LegacyShapeletTransform();
                    shapeletFilterCV.setQualityMeasure(QualityMeasures.ShapeletQualityChoice.INFORMATION_GAIN);
                    shapeletFilterCV.supressOutput();
                    shapeletFilterCV.setNumberOfShapelets(instancesTrain[dataIndex].numAttributes()/2);
                    shapeletFilterCV.setShapeletMinAndMax(shapeletMinMax[dataIndex][0], shapeletMinMax[dataIndex][1]);
                    shapeletFilterCV.setLogOutputFile(filePaths[dataIndex]+fileNames[dataIndex]+"_TRANS_"+n+".txt");
                }

                //Transform data
                try{
                    trainFold = shapeletFilterCV.process(trainFold);
                    testFold = shapeletFilterCV.process(testFold);
                }catch(Exception e){
                    e.printStackTrace();
                }
            }
    ...
    */
    ...
    public static void main(String[] args) throws Exception{
        ...
        tableToProduceIndex = 4;
        outFileName = "";
        fileToProcessIndex = 1;
        classifierToProcessIndex = 2;
        // Process user input from arguments.
        try{
            ...
            tableToProduceIndex = 4;        // refer to paper for indices    
        ...
        try{
            switch (tableToProduceIndex){
                ...
                default: throw new IOException("Unknown table identifier.");
            }
        }catch(Exception e){
            e.printStackTrace();
        }
